...
- **구매내역 기반 재고 등록**: OpenAI 비전으로 배달의민족 장보기·쿠팡·마켓컬리 주문내역과 범용 전자영수증의 텍스트를 분석합니다. 웹 파일 탐색기와 Android 사진 보관함에서 최대 10장을 선택하고, 재고명·수량·권장 소진일을 검수한 뒤 한 번에 등록합니다.
- **학습형 권장 소진일 캐시**: 정규화된 식품의 공용 권장 기간을 먼저 재사용하고, 없는 항목만 근거를 웹 검색해 저장하므로 사용이 쌓일수록 반복 검색을 줄입니다.
...
| 10종 게스트 재고·임박 재료·정렬 | Supabase 원격 조회 및 로컬 검증 모드 동작 완료 |
| 레시피 상세·타이머·재조리·재고 차감 | Supabase 원자적 차감 및 동일 요청 중복 방지 검증 완료 |
